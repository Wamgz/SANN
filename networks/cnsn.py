...
def calc_ins_mean_std(x, eps=1e-5):
    ...
    assert (len(size) == 4)
    N, C = size[:2]
    # torch.Size([2, 3])， 通道上的方差tensor([[0.9969, 0.9907, 0.9995], [0.9936, 1.0116, 1.0078]])
...
class CrossNorm(nn.Module):
    """CrossNorm module"""
    def __init__(self, active=False, crop=None, beta=None):
        super(CrossNorm, self).__init__()

        self.active = active

    def forward(self, x):
        indexs = torch.arange(x.size()[0]).to(x.device)
        if self.training and self.active:
            x, indexs = cn_op_2ins_space_chan(x)
        return x, indexs


class SelfNorm(nn.Module):
    """SelfNorm module"""
    def __init__(self, chan_num, is_two=False, active=False):
    ...
    def forward(self, x):
        ...
        mean, std = calc_ins_mean_std(x, eps=1e-12) # 通道维度上的，mean：torch.Size([2, 3, 1, 1]) var：torch.Size([2, 3, 1, 1])

        statistics = torch.cat((mean.squeeze(3), std.squeeze(3)), -1) # torch.Size([2, 256, 2])

        g_y = self.g_fc(statistics) # torch.Size([2, 3, 1])
        g_y = self.g_bn(g_y) # 
...
class CNSN(nn.Module):
    ...
    def __init__(self, crossnorm, selfnorm, active=True):
    ...
    def forward(self, x):
        ...
        if self.crossnorm and self.crossnorm.active:
            x, indexs = self.crossnorm(x)
        if self.selfnorm and self.selfnorm.active:
            x = self.selfnorm(x)
        return x, indexs


if __name__ == '__main__':
    x = torch.randn(5, 16, 4, 4)
    model = CNSN(CrossNorm(active=True), SelfNorm(16))
    x, indexes = model(x)
    print(x.size())
    print(indexes)